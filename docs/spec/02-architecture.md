...
| Google Cast | Media3 Cast + Cast SDK | 1.9.2 / 22.3.0 |
...
4. **Playback Layer** — `PlaybackService` (MediaSessionService) manages ExoPlayer and CastPlayer
...
User Enables Cast
  → CastPlayer replaces ExoPlayer
  → On failure → fallback to "Open in YouTube"
```
